...
public class EmployeeDAO {
	...
	public List getEmployeeList(EmployeePersonalInfo info) throws Exception {
		...
		try {
			...
			//pst.setString(1,"".equals(info.getPfID())?"%":info.getPfID());
			//pst.setString(2,"".equals(info.getPfID())?"%":info.getPfID());
			log.info("PFID " +info.getPfID().trim());
			pst.setString(1,info.getPfID().trim());
			pst.setString(2,info.getPfID().trim());
			pst.setString(3,info.getRegion()+"%");
			pst.setString(4,"%"+info.getEmployeeName()+"%");
            rs = pst.executeQuery();
