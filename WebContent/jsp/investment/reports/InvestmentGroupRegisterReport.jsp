...
<%@ page import="com.epis.dao.investment.InvestmentReportDAO" %>
...
<%
	if("excel".equals(request.getParameter("reportType"))){
		...
		response.setHeader("Content-Disposition", "attachment; filename=investmentgroupregisterreport.xls");
	}
%>
					...
					<table width=100%>
						...
						<tr>
							...
							<td align="center" class="tblabel">
								<bean:message key="report.interestreceived" bundle="common"/>
							</td>
							...
							<td align="center" class="tblabel">
								<bean:message key="report.maturityreceived" bundle="common"/>
							</td>
						...
						<c:set var="secproposal" value=""/>
						<c:set var="half" value="Half-Yearly"/>
						
						 <c:forEach var='arranger' items='${arrangerList}'> 
						 		<c:if test="${(arranger.securityProposal!=secproposal)}">
                                 <tr >
                                 <bean:define name="arranger" property="groupName" id="checkgroup" type="java.lang.String"/>
                                 <td class="tableText" align="left" nowrap><%=cnt++%></td> 
                                 <td class="tableText" align="left" nowrap><c:out value='${arranger.investmentdate}'/></td> 
                                 <td class="tableText" align="left"  nowrap><c:out value='${arranger.securityName}'/></td>
                                 <c:if test="${(arranger.groupName!=group)}">
                                 <td class="tableText" align="left" rowspan="<%=InvestmentReportDAO.getInstance().getGroupCount(checkgroup)%>" nowrap><c:out value='${arranger.groupName}'/></td>
                                 </c:if>  
                                 ...
                                 <td class="tableText"  align="right" nowrap><bean:define id="facevalue" name="arranger" property="faceValue" type="java.lang.String"/><%=cf.getDecimalCurrency(Double.parseDouble(facevalue))%></td> 
                                 ...
                                 <td class="tableText" align="right" nowrap><bean:define id="invfacevalue" name="arranger" property="investmentFaceValue" type="java.lang.String"/><%=cf.getDecimalCurrency(Double.parseDouble(invfacevalue))%></td> 
                                ...
                                <td class="tableText"  align="right"  nowrap><%=cf.getDecimalCurrency(Double.parseDouble(annualinterest)/2)%></td>  
                                 ...
                                 <c:if test="${(arranger.groupName!=group)}">
                                 <td class="tableText" align="right" rowspan="<%=InvestmentReportDAO.getInstance().getGroupCount(checkgroup)%>"><bean:define id="interestreceived" name="arranger" property="interestrvincb" type="java.lang.String"/><%=sq.newLineAddress(interestreceived)%></td>
                                 </c:if>
                                 <td class="tableText" align="left" nowrap><c:out value='${arranger.maturityDate}'/></td>  
                                 <c:if test="${(arranger.groupName!=group)}">  
                                 <td class="tableText" align="left"  rowspan="<%=InvestmentReportDAO.getInstance().getGroupCount(checkgroup)%>"><c:out value='${arranger.maturityincb}'/></td> 
                                 </c:if> 
                               </tr>
                               </c:if>
                               <c:remove var="group" scope="page"/>
                               <c:remove var="secproposal" scope="page"/>
                               <c:set var="group" value='${arranger.groupName}'/>
                               <c:set var="secproposal" value='${arranger.securityProposal}'/>
