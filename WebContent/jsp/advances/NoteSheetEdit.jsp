	...
	function submitData(st,frmName,continueFlag){
		...
		//	}			
			...
			//alert('Station name is different');
			 ...
			 if (chkstatus== true)
			 {
			 var url ="<%=basePathWoView%>editNoteSheet.do?method=updateNoteSheet&frm_name="+frmName+"&frmFlag="+flag+"&bankflag="+bankFlag+"&frm_nominees="+encodeURIComponent(te)+"&continueFlag="+continueFlag;							 
			document.forms[0].action=url;
			document.forms[0].method="post";
			document.forms[0].submit();		
			 }
	 ...
	 function loadDets(){		     
		  //alert(wthdrwdets);
		   ...
		   for(var i=0;i<nomineearray.length;i++){
		    ...
		    // alert("loadDetssssssss=========="+nomineearrfinal[0]);
...
<body class="bodybackground" onload="hide('<%=request.getAttribute("focusFlag")%>');getNominee();loadAdhoc();loadBankDetails();loadDetails('<%=request.getAttribute("nomineeInfo")%>','<bean:write name="advanceBean" property="frmName"/>')">
		<html:form method="post" action="editNoteSheet.do?method=updateNoteSheet">
				<%=ScreenUtilities.screenHeader("Final Settlement Process Form")%>
