...
<html>
	<head>
		...
		function resetMaster(){
			...
			var url="<%=basePath%>loadApproval.do?method=loadCheckListSearch&flag=PFWCheckList";
		   ...
		   if(document.forms[0].chk.length!=undefined){    
		      ...
		      if(count==0){
		         ...
		         for(var j=0;j<str.length;j++){			         
			        ...
			        if(frmType=='CheckList'){
			            if(verifiedBy==''){
			        	checklistApprove(transID,pensionNo,advanceType,status)
			        	}else{
			        	  alert('Advance request with transaction id '+transID+' is already approved by Personnel Department.');
			        	}
		...
		function checklistApprove(transID,pensionNo,advanceType,status){
		
		var flag;		 
		    if(status=='Y'){
		    
		     var chkstatus= confirm("Check List is already approved.Do you really want to edit?");
			
			 if (chkstatus==true){			 
			 flag='ApprovalEdit';		   
			 	var url="<%=basePath%>checklist.do?method=checkListApprove&frmName=PFWCheckList&frmPensionNo="+pensionNo+"&frmTransID="+transID+"&frmAdvStatus="+status+"&frmFlag="+flag;
	   ...
	   var flag='ApprovalReport';	
		    ...
		    var url="<%=basePath%>checklist.do?method=checkListApprove&frmName=PFWCheckListReport&frmPensionNo="+pensionNo+"&frmTransID="+transID+"&frmAdvStatus="+status+"&frmFlag="+flag;
			wind1 = window.open(url,"PFWCHECKLIST","toolbar=yes,statusbar=yes,scrollbars=yes,menubar=yes,resizable=yes,width="+swidth+",height="+sheight+",top=0,left=0");
	   	 	winOpened = true;
			wind1.window.focus();
			}
		...
		function search()
		{
			...
			var url="<%=basePath%>loadApproval.do?method=search23Advances&frmName=PFWCheckList";
		 	document.forms[0].action=url;
			document.forms[0].method="post";
			document.forms[0].submit();
	...
	<body onload="document.forms[0].advanceTransID.focus();">
		<html:form method="post" action="/advanceSearch.do?method=loadAdvanceSearchForm">
			<%=ScreenUtilities
									.saearchHeader("loansadvances.pfwchecklist.screentitle")%>
					...
					<td height="15%">
						<table align="center">
							<tr>
								...
								<td>
									<html:select property="advanceTransStatus" style="width:119px" styleClass="TextField">
										<html:option value=''>Select One</html:option>
										<html:option value='N'>N</html:option>
										<html:option value='A'>Y</html:option>
				...
				<tr>
					<td width="37%" height="29" align="left" valign="top">
						<table width="155" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="40">
									<input type="button" value="Check List Form" class="btn" onclick="javascript:selectSubMenuOptions('CheckList')">
								</td>
								<td width="40">
					...
					<tr>
						<td align="center" width="100%">
							<display:table cellspacing="0" cellpadding="0" class="GridBorder" style="width:100%;height:50px" export="true" sort="list" id="advanceList" sort="list" pagesize="10" name="requestScope.searchlist"
								requestURI="./loadApproval.do?method=search23Advances&amp;frmName=PFWCheckList">
