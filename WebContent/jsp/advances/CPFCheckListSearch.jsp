...
<html>
	<head>
		...
		function resetMaster(){
			 ...
			 document.forms[0].checkListStatus.value="";			 
			var url="<%=basePath%>loadApproval.do?method=loadCheckListSearch&flag=CPFCheckList";
		      ...
		      if(count==0){
		         ...
		         for(var j=0;j<str.length;j++){			         
			        ...
			        if(frmType=='CheckList'){
			        	checklistApprove(transID,pensionNo,advanceType,status,verifiedby)
			        }else if(frmType=='Print'){
			        	advancesForm2Report(transID,pensionNo,advanceType,status)
			        }
		   	
		}
		function checklistApprove(transID,pensionNo,advanceType,status,verifiedby){
		...
		return false;
			...
			var flag;		 
			 ...
			 if(verifiedby=='PERSONNEL'){
		         alert("CPF Advance Request Already Verified");
			    ...
			    if(status=='Y'){
				 ...
				 flag='ApprovalEdit';		   
					...
					var url="<%=basePath%>checklist.do?method=advanceCheckListApprove&frmName=CPFCheckList&frmPensionNo="+pensionNo+"&frmTransID="+transID+"&frmAdvStatus="+status+"&frmFlag="+flag;
					document.forms[0].action=url;
					document.forms[0].method="post";
					document.forms[0].submit();
				 }
	   ...
	   var flag='ApprovalReport';	
		...
		function search()
		{
			...
			var url="<%=basePath%>loadApproval.do?method=search23Advances&frmName=CPFCheckList";
		 	document.forms[0].action=url;
			document.forms[0].method="post";
			document.forms[0].submit();
	...
	<body onload="document.forms[0].advanceTransID.focus();">
		<html:form method="post" action="/advanceSearch.do?method=loadAdvanceSearchForm">
			<%=ScreenUtilities.saearchHeader("loansadvances.cpfchecklist.screentitle")%>
					...
					<td height="15%">
						<table align="center">
							<tr>
								...
								<td>
									<html:select property="checkListStatus" style="width:119px" styleClass="TextField">
