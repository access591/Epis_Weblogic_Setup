<!--
/*
  * File       : NoteSheetArrearSearch.jsp
  * Date       : 27/05/2010
  * Author     : Suneetha V 
  * Description: 
...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function resetMaster(){		
			...
			url="<%=basePath%>noteSheetSearch.do?method=loadFinalSettlementArrearSearchForm";
		...
		{		   
			var url="<%=basePath%>noteSheetSearch.do?method=searchFinalSettlementArrear&frm_name=FSArrForm";
		 	document.forms[0].action=url;
			document.forms[0].method="post";
			document.forms[0].submit();	
		}
		    ...
		    if(document.forms[0].chk.length!=undefined){    
		      ...
		      if(count==0){
		        ...
		        for(var j=0;j<str.length;j++){			         
			        ...
			        editFinalSettlementArrear(pensionno,nssanctionno,transactionStatus,frmName);
		}
		
			function editFinalSettlementArrear(pensionno,nssanctionno,transactionStatus,frmName)
		{  //alert(transactionStatus);	
			    	    
		    if(transactionStatus=='FINANCE'){
		      alert('Application No '+nssanctionno+' already approved by Finance Department. You can not edit the details.');
		    }else if(transactionStatus=='FINANCE,SRMGRREC'){
		      alert('Application No '+nssanctionno+' already approved by Senior Manager. You can not edit the details.');
		    }else if(transactionStatus=='FINANCE,SRMGRREC,DGMREC'){
		      alert('Application No '+nssanctionno+' already approved by DGM. You can not edit the details.');
		    }else if(transactionStatus=='FINANCE,SRMGRREC,DGMREC,APPROVED'){
		      alert('Application No '+nssanctionno+' already approved . You can not edit the details.');
		    }else{
			var url="<%=basePath%>loadNoteSheet.do?method=editFinalSettlementArrear&frmPensionNo="+pensionno+"&frmSanctionNo="+nssanctionno+"&frm_name="+frmName;
		...
		if(verifiedBy=='FINANCE,SRMGRREC,DGMREC,APPROVED'){
		 alert('You can not Delete the details.');		 
		    ...
		    var chkstatus= confirm("Do you really want to delete Final Settlement Arrear Request?");
  ...
  <%=ScreenUtilities.saearchHeader("loansadvances.finalsettlementarrearsearch.screentitle")%><table width="550" border="0" cellspacing="3" cellpadding="0">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
               ...
               <tr>
                   <td  height="29" align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>  
							<td>								
								<input type="button" value="New" class="btn" onclick="javascript:window.location.href='<%=basePath%>loadNoteSheet.do?method=loadFinalSettlementArrear'">
							</td>
							<td>
