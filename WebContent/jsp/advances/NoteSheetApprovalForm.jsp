<!--
/*
  * File       : NoteSheetApprovalFormIV.jsp
  * Date       : 19/04/2010
  * Author     : Suneetha V
  * Description: 
...
<html>
	...
	function submitData(st,frmName){
			...
			var postingflag=document.forms[0].postingFlag.value;
			var postingstation=document.forms[0].postingStation.value;
			var postingregion=document.forms[0].postingRegion.value;
		...
		//	}			
			...
			if(verifiedby=='PERSONNEL,FINANCE,SRMGRREC,DGMREC'){
			   flag="NewForm";
			}else{
				flag="EditForm";		
			}
			...
			//alert('Station name is different');
			 ...
			 if (chkstatus== true)
			 {
			 var  url ="<%=basePathWoView%>editNoteSheet.do?method=updateNoteSheet&frm_name="+frmName+"&frmFlag="+flag+"&frm_nominees="+encodeURIComponent(te)+"&posting_region="+postingregion+"&posting_station="+postingstation+"&posting_flag="+postingflag+"&bankflag="+bankFlag;			
			document.forms[0].action=url;
			document.forms[0].method="post";
			document.forms[0].submit();		
			 }
 ...
 function callNominee(id,nomineeName,nomineeDOB,nomineeAddress,nomineeRelation,gardianName,gardianAddress,totalShare){     
	...
	/* if(reclength>=1){
	    if(validateNomineeDetails(reclength)!=true){
			return false;
	    }
    }
    reclength++; */
   ...
   loadIDList.push(reclength);     
	...
	inp4.setAttribute("maxlength", "12"); 
	if(nomineeRelation!=''){
...
<body class="bodybackground" onload="loadBankDetails();hide('<%=request.getAttribute("focusFlag")%>');getNominee();loadAdhoc();loadDetails('<%=request.getAttribute("nomineeInfo")%>','<bean:write name="advanceBean" property="frmName"/>')">
		<html:form method="post" action="editNoteSheet.do?method=updateNoteSheet">
				<%=ScreenUtilities.screenHeader("Final Settlement Approval Form")%>
		<table width="720" border="0" cellspacing="3" cellpadding="0">
										...
										<tr >										     
											<td  class="tableTextBold" >Name of Employee as per <br/>Saving Bank Account&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
										...
										<logic:iterate	id="bankList" name="bankMasterBeanList" >	
										<tr>										     
											
											<td><html:text styleClass="TextField"  size="20%" property="bankempname" name="bankList"/></td>
											 
											
											<td><html:text styleClass="TextField"  size="20%" property="bankname" name="bankList"/></td>
															   
											
											<td><html:text styleClass="TextField"  size="20%" property="banksavingaccno" name="bankList"/></td>
																						
											
											<td><html:text styleClass="TextField"  size="20%" property="bankemprtgsneftcode" name="bankList"/></td>
										</tr>
										 </logic:iterate>
										...
										<tr id="remarks">
											...
											<td colspan="3">
											   ...
											   <html:textarea property="remarks"  name="noteSheetInfo" tabindex="37" rows="7" cols="60"  onkeypress="chngRemarks();" />
												...
												<html:hidden property="postingFlag" name="noteSheetInfo"/>
										<html:hidden property="postingStation" name="noteSheetInfo"/>
										<html:hidden property="postingRegion" name="noteSheetInfo"/>					
