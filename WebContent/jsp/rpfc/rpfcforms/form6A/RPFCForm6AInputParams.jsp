...
<html>
 ...
 function LoadWindow(params){
	...
	function resetReportParams(){
		document.forms[0].action="<%=basePath%>rpfcforms.do?method=loadrpfcform6a";
	...
	//	if(monthID!='NO-SELECT'){
		...
		params = "&frm_region="+regionID+"&frm_year="+yearID+"&frm_reportType="+reportType+"&frm_empnm="+empName+"&frm_emp_flag="+frm_empflag+"&frm_pensionno="+empserialNO+"&frm_pfids="+pfidStrip+"&sortingOrder="+sortingOrder+"&frm_ToYear="+toYearID+"&frm_month="+monthID+"&frm_airportcode="+airportcode;
		url="<%=basePath%>rpfcforms.do?method=rpfcform6areport"+params;
...
function getPFIDNavigationList()
	...
	<body class="BodyBackground" onload="javascript:frmload()">
		<%String monthID = "", yearDescr = "", region = "", monthNM = "";
            ...
            %>
		<form action="post">
		<%=ScreenUtilities.screenHeader("Form-6A Input Params")%>
