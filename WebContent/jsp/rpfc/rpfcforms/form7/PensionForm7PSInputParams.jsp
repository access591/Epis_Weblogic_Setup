...
<html>
 ...
 function LoadWindow(params){
	...
	function resetReportParams(){
		document.forms[0].action="<%=basePath%>rpfcforms.do?method=loadform7Input";
	...
	function validateForm() {
		var regionID="",toYearID="",empName="",airportcode="",params="",reportType="",url="",yearID="",monthID="",yearDesc="",formType="",toYearID="",formType="";
		...
		if (document.forms[0].chk_empflag.checked==true){
	       		  ...
	       		  frm_empflag=false;
       	       	...
       	       	if(yearID=='NO-SELECT' && formType=='FORM-7PS-REVISED'){
       		alert('User should be select Year');
				document.forms[0].select_year.focus();
				return false;
       	}
		params = "&frm_region="+regionID+"&frm_year="+yearID+"&frm_month="+monthID+"&frm_reportType="+reportType+"&frm_empnm="+empName+"&frm_emp_flag="+frm_empflag+"&frm_reportType="+reportType+"&frm_formType="+formType+"&frm_pensionno="+empserialNO+"&frm_pfids="+pfidStrip;
	
		if(formType=='FORM-7PS'|| formType=='FORM-7PS-SUMMARY'|| formType=='FORM-7PS-REVISED'){
			url="<%=basePath%>rpfcforms.do?method=form7report"+params;
			
		}else if(formType=='FORM-7PS-NO-DATA'){
			url="<%=basePath%>rpfcforms.do?method=form7reportzero"+params;
		}else if(formType=='FORM-7PS-INDEXPAGE'){
			url="<%=basePath%>rpfcforms.do?method=form7Indexreport"+params;
		}
...
function getPFIDNavigationList()
	...
	<body class="BodyBackground" onload="javascript:frmload()">
		<%String monthID = "", yearDescr = "", region = "", monthNM = "";
            ...
            %>
		<form action="post">
		<%=ScreenUtilities.screenHeader("Form-7 Report Params")%>
			...
			<table width="75%" border="0" align="center" cellpadding="0" cellspacing="0" class="tbborder">
				...
				<tr>
					<td class=tableTextBold align="right" nowrap>Form Type:<font color=red>*</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
