...
String space = "<tr><td colspan=8 style='height:10px'></td></tr>";
				...
				<td align="center" >
					<table border="0" cellpadding="3" cellspacing="0" align="center" valign="middle">
						...
						<tr>
							...
							<%=sql.getDescription("cb_bank_info","TRUSTTYPE","ACCOUNTCODE",request.getParameter("accountHead"),DBUtility.getConnection())+" TRUST"%>
							</td>
						</tr>
						<tr>
							<td align="center" class="tblabel" >
						...
						<tr>
							<td align="center" class="tblabel" >
							<%="("+sql.getDescription("cb_bank_info","BANKNAME||' - '||ACCOUNTNO","ACCOUNTCODE",request.getParameter("accountHead"),DBUtility.getConnection())+")"%>
							</td>
						</tr>
						<tr>
							<td align="center" class="tblabel" >
...
%>
						...
						<tr>
							...
							<td>
								<table cellpadding="0" cellspacing="0" border="1" width="100%" style='border-color: black; border-width: 1px' >
									<thead>
									<tr>
...
for(int i=0;i<size;i++){	
	...
	if(recCount == 1){
		if(amount > 0){
			credit = df.format(amount);
			debit = "-";
		}else {
			debit = df.format(Math.abs(amount));
			credit = "-";
		}
		vDetails ="<td class='Data'  rowspan='"+recs+"' width='70px'>"+book.getVoucherDate()+"</td><td class='Data'  rowspan='"+recs+"' nowrap align=left >"+book.getVoucherno()+"</td><td class='Data'  rowspan='"+recs+"' nowrap align=left >"+book.getChequeNo()+"</td>";
		details = "<td class='Data' align=left rowspan='"+recs+"' >"+book.getPartyName()+"<BR><I>"+sql.wrapTextArea("("+book.getDescription()+")",20)+"</td>";
		payRecep = "<td class='Data' align='right' nowrap rowspan='"+recs+"'>"+credit+"</td><td class='Data' align='right' rowspan='"+recs+"' nowrap>"+debit+"</td>";
	}
	if(recs==recCount){
		recCount=1;
		linespace = space;
...
%>
									...
									<tr>
										<td class="label" align="right" colspan="6">
											Closing balance(<%=(payments<receipts?" Dr. ":" Cr. ")%>)
										</td>
										<td class="label" align="right" >
											<%=payments<receipts?df.format(Math.abs(payments-receipts)):""%>
										</td>
										<td class="label" align="right" >
											<%=payments>receipts?df.format(Math.abs(payments-receipts)):""%>
										</td>
									...
									<tr>
										<td class="label" align="right" colspan="6">
											Opening balance(<%=(payments>receipts?" Dr. ":" Cr. ")%>)
										</td>
										<td class="label" align="right" >
											<%=payments>receipts?df.format(Math.abs(payments-receipts)):""%>
										</td>
...
<%
	...
	String amountDt = df.format(book.getVoucherno().startsWith("C")?book.getReceipts():book.getPayments())+" CR";
	if("0.00 CR".equals(amountDt)){
		amountDt = df.format(book.getVoucherno().startsWith("C")?book.getPayments():book.getReceipts())+" DR";
	}
%>
									<tr>
										<%=vDetails%>
										<td class="Data"  align=left  nowrap="wrap">
											<%=(book.getAccountHead()+" "+book.getParticular()).trim()%><%=details%>										
...
%>									
									<tr>
										...
										<td class="label" align="right" >
											<%=debitCB==0.00?"":df.format(debitCB)%>
										</td>
										<td class="label" align="right" >
											<%=creditCB==0.00?"":df.format(creditCB)%>
										</td>
									</tr>				
									<tr>
										<td class="label" align="right" colspan="6">
										...
										<td class="label" align="right" >
											<%=df.format(grandTotDebit)%>
										</td>
										<td class="label" align="right" >
											<%=df.format(grandTotCredit)%>
										</td>
									</tr>
								</table>
							</td>
						...
						<tr>
							<td align="center">
								<table width="75%" cellpadding="0" cellspacing="0">
									...
									<tr>
										<td class="Data" align="center" width='33%' colspan=2>
											Certified
										</td>
										<td class="Data" align="center" width='33%' colspan=3>
											Cashier
										</td>
										<td class="Data" align="center" width='33%' colspan=3>
											Manager(Finance)
