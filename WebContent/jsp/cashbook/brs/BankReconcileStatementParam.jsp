...
<html>
		   ...
		   function checkRecord()
		   {
		   	...
		   	if(document.forms[0].accountNo.value == ""){
				...
				if(!convert_date(document.forms[0].voucherDt)){
		   		...
		   		var swidth=screen.Width-10;
				var sheight=screen.Height-150;
				var url = "/voucher.do?method=bankreconcileReport&&voucherDt="+document.forms[0].voucherDt.value+"&&accountNumber="+document.forms[0].accountNo.value;
				window.open(url,"BankReconcileStatement","toolbar=yes,statusbar=yes,scrollbars=yes,menubar=yes,resizable=yes,width="+swidth+",height="+sheight+",top=0,left=0");
				return false;
		   }
	...
	<body>
		<html:form  method="post" action="/voucher.do?method=bankReconciliation" onsubmit="javascript:return checkRecord()">
			<%=ScreenUtilities.screenHeader("brs.bankreconcilestatement")%>
